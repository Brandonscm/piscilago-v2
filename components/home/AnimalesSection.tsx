  ...
  return (
    <section className="mt-4">
      <div className="flex items-center justify-between px-4 mb-2.5">
        ...
        <Link href="/huellas" className="inline-flex items-center gap-0.5 text-[10px] text-wild-600 font-semibold uppercase tracking-wide active:scale-95 transition-transform">
          Ver insignias
      ...
      <div ref={scrollRef} className="flex gap-2.5 overflow-x-auto no-scrollbar px-4 pb-2 scroll-smooth">
        {SPECIES.map((s) => (
          ...
          >
            ...
            <div className="p-2.5">
              <p className="text-[11px] font-semibold text-ink-900 leading-tight">{s.name}</p>
              <p className="text-[9px] text-ink-500 mt-0.5 italic truncate">{s.scientific}</p>
            </div>
          </Link>
        ))}
      </div>
