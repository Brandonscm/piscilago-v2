...
function imageFor(id: string): string {
  return (
    ATTRACTION_IMAGES[id] ??
    "https://images.unsplash.com/photo-1530841344095-502dd5b15909?w=400&q=70"
  );
}

export function AtraccionesSection({ attractions }: { attractions: LiveAttraction[] }) {
  const featured = attractions
    .filter((a) => a.type === "tobogan" || a.type === "piscina" || a.type === "rio")
    .slice(0, 6);
  ...
  return (
    <section className="mt-5">
      <div className="flex items-center justify-between px-4 mb-2.5">
        ...
        <Link
          href="/filas"
          className="inline-flex items-center gap-0.5 text-[10px] text-col-600 font-semibold uppercase tracking-wide active:scale-95 transition-transform"
        >
          Ver todas
      ...
      <div className="flex gap-2.5 overflow-x-auto no-scrollbar px-4 pb-2">
        {featured.map((a) => {
          const stripe =
            a.congestion === "high" ? "bg-status-red" :
          ...
          return (
            ...
            >
              ...
              <div className="p-2.5">
                <p className="text-[11px] font-semibold text-ink-900 leading-tight truncate">
                  {a.name}
                </p>
                <p className="text-[9px] text-ink-500 mt-0.5">
                  {a.occupancyPct}% aforo · zona {a.zone}
                </p>
              </div>
            </Link>
          );
